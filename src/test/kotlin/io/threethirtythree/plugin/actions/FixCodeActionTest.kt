...
class FixCodeActionTest {
    ...
    private lateinit var action: FixCodeAction
    ...
    fun testActionPerformedWithValidSelection() {
        ...
        lenient().`when`(mockSelectionModel.selectedText).thenReturn("int x = 1/0;")
        ...
        mockStatic(Messages::class.java).use { messagesMock ->
            ...
            messagesMock.verify({
                Messages.showWarningDialog(
                    eq(mockProject),
                    eq("Please select some code to analyze for bugs."),
                    eq("No Code Selected"),
                )
            })
        }
    ...
    fun testUpdateWithValidSelection() {
        ...
        `when`(mockSelectionModel.selectedText).thenReturn("int x = 1/0;")
