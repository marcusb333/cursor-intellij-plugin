...
# Cursor AI IntelliJ Plugin Build Script (No Wrapper)
# This script provides common build operations using global Gradle installation
...
# Wrapper for Gradle with common flags (using global installation)
gradle_cmd() {
    gradle --stacktrace --no-daemon "$@"
}
...
check_prerequisites() {
    ...
    # Check global Gradle installation
    if command -v gradle &> /dev/null; then
        GRADLE_VERSION=$(gradle --version 2>/dev/null | grep "Gradle" | head -n 1 || true)
        print_success "Global Gradle installation found: $GRADLE_VERSION"
    else
        print_error "Global Gradle installation not found. Please install Gradle globally."
        print_warning "Install with: brew install gradle (on macOS) or visit https://gradle.org/install"
        exit 1
    fi
...
This script uses the global Gradle installation instead of the Gradle wrapper.
Make sure you have Gradle installed globally before using this script.
...
Prerequisites:
  - Global Gradle installation (install with: brew install gradle)
  - Java 21+ runtime
  - IntelliJ Platform dependencies (auto-downloaded by Gradle)

Note: This script uses the global Gradle installation instead of the wrapper.
      If you prefer using the wrapper, use the original build.sh script.
EOF
}
